...
var model = 'https://api.projectoxford.ai/luis/v1/application?id=c413b2ef-382c-45bd-8ff0-f76d60e2a821&subscription-key=71cd924cf4d04f82ba6c2b1fd1585530&q=';
var recognizer = new builder.LuisRecognizer(model);
var dialog = new builder.IntentDialog({ recognizers: [recognizer] });
bot.dialog('/', dialog);
var profileDialog = new builder.IntentDialog({ recognizers: [recognizer] });
bot.dialog('/profile', profileDialog);
...
dialog.onDefault((session) => {
    session.send("I'm sorry I didn't understand. I can only create & delete alarms.");
    checkName(session);
});
dialog.onBegin((session, args, next) => {
//    session.send("I'm DeathAngel");
//    builder.DialogAction.send("I'm DeathAngel");
    next();
});



profileDialog.onBegin([
//    (session) => {builder.Prompts.text("What's your name?");},
    (session) => {
